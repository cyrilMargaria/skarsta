...
// updated and changed by Cyril margaria 2020 (shaft height, flat spot, hex key)

// Based on pulleys by:
// https://www.thingiverse.com/thing:2758711 by Oscar January
// http://www.thingiverse.com/thing:11256 by Oscar January
...
idler_bolt_option = 9; // [0:none, 1:M3, 2:M4, 3:M5, 4:M6, 5:M8, 6:M10, 7:M12, 8:custom 9:hex_6mm] for custom: make changes below!
...
bolt_head_diameters = [0.000, 6.010, 7.660, 8.79, 11.05, 14.38, 17.77, 20.03,  15.8, 7]; // circle diameter, not width across flats!
bolt_head_heights =   [0.000, 2.125, 2.925, 3.65,  4.15,  5.45,  6.58,  7.68,  5.5, 6 ];
      ...
      difference(){   
        translate([0,0,idler_height]){
          union(){
            ...
            if ( belt_retainer == 1 ){
              translate ([0,0, base_height + toothed_part_length ]){
                ...
                rotate_extrude($fn=number_of_teeth*20){
                  square([pulley_OD/2+idler_height,idler_height]);
            ...
            // MMMM
            translate([0,0,base_height + toothed_part_length + retainer_height]) cylinder(d=idler_base_diameter, h=total_height-base_height - toothed_part_length - retainer_height, $fn=motor_shaft*20);
          } // end of union
        ...
        translate([0,0,-1]){
          ...
          difference() {          
          cylinder(r=motor_shaft/2,h=motor_shaft_h + 1.2 ,$fn=motor_shaft*20);
          translate([motor_shaft/2-motor_shaft_flat,-motor_shaft/2,1+motor_shaft_h-motor_shaft_flat_h+0.2]) cube([motor_shaft_flat, motor_shaft, motor_shaft_flat_h]);
          }
        }
        ...
        // Revove hexagon
        if (bolt_head_diam > 0){
          translate([0,0,motor_shaft_h]){
            cylinder(d=bolt_head_diam,h=total_height+0.1,$fn=6);
          }
        }
      }
//    }
